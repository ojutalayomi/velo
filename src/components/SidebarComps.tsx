        ...
        {sidebarItems.slice(3, 8).map((item, key) => (
          <Link key={item.route+key+'l'} aria-label={item.route} className='hidden' href={`/${item.route}`}>
            ...
            >
              ...
              {(item.label === 'Chats' && i > 0) && <div className='bg-brand text-white text-xs absolute right-0 transform translate-y-[-50%] md:translate-y-0 md:relative font-bold rounded-full w-5 h-5 flex items-center justify-center'>{i}</div>}
            </div>
          </Link>
        ))}
      </PopoverContent>
    ...
    <Link href={`/${route}`}>
      ...
      >
        ...
        >
          ...
          {(item.label === 'Chats' && i > 0) && <div className={`bg-brand text-white text-xs absolute transform translate-x-[50%] translate-y-[-50%] ${!isCollapsed && 'md:translate-y-0 md:relative'} font-bold rounded-full w-5 h-5 flex items-center justify-center`}>{i}</div>}
        </div>
      </div>
    </Link>
  );
...
export const UserSection = forwardRef<HTMLDivElement, {
  ...
  handlePopUp: () => void;
  refetchUser: () => void;
}>(({ error, loading, userdata, pathname, isPopUp, isCollapsed = false, handlePopUp, refetchUser }, ref) => (
  <div ref={ref} className='flex'>
    {!userdata._id ? (
      loading 
        ? 
        <UserComponentLoading loading={loading} error={error} isCollapsed={isCollapsed} refetchUser={refetchUser}/> 
        : 
...
const UserComponentLoading = ({loading, error, isCollapsed = false, refetchUser}: {loading: boolean, error: boolean, isCollapsed?: boolean, refetchUser: () => void}) => {
  return (
    <div className='user md:items-center hover:bg-slate-200 dark:hover:bg-neutral-900 !justify-center md:!justify-between !my-[.5em] !mx-0 !px-2 !py-1 md:shadow-bar md:dark:shadow-bar-dark w-full'>
        ...
        <div className={`flex-1 items-center max-w-[50%] names flex-col hidden ${isCollapsed ? 'hidden' : 'md:!flex'}`}>
        ...
        <Ellipsis size={25} className={`cursor-pointer hidden ${isCollapsed ? 'hidden' : 'md:!block'} dark:text-gray-400`} />
    </div>
  )
}

const UserComponent = ({userdata, isCollapsed = false}: {userdata: UserData, isCollapsed?: boolean}) => {
  return (
    <div className={`${isCollapsed ? '!shadow-none' : ''} user md:items-center hover:bg-slate-200 dark:hover:bg-neutral-900 !justify-center md:!justify-between !my-[.5em] !mx-0 !px-2 !py-1 md:shadow-bar md:dark:shadow-bar-dark w-full`}>
      ...
      <div className={`flex-1 items-center max-w-[50%] names hidden flex-col ${isCollapsed ? 'hidden' : 'md:flex'}`}>
        <div className='flex max-w-full dark:text-slate-200'>
