...
export default async function handle(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const followerDetails = await db.users().findOne({ _id: new ObjectId(followerId) }, { projection });
        ...
        if (follow) {
            ...
            // Emit follow event
            const followedDetails = await db.users().findOne({ _id: new ObjectId(followedId) }, { projection });
