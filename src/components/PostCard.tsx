    ...
    <div className='pre-blog' id={data.PostID.slice(0,-4)}>
      <div className='blog !dark:shadow-bar-dark select-none dark:bg-zinc-900 shadow-md dark:text-slate-200' data-id={data.PostID}>
        ...
        {(data?.Image.length > 0 && showMedia) &&
          <MediaSlide postData={data} className={`${containsPost ? '' : 'max-h-96'} ${data?.Image.length > 0 ? 'rounded-xl overflow-hidden mb-2' : ''}`} isLink/>
        }
        {/* Post Card */}
        {originalPost  && (
        ...
        {containsPost ? (
          <>
          <div className='blog-time border-t pt-1'>{timeFormatter(data.TimeOfPost).replaceAll(':',' : ')}</div>
