...
async function createChat() {
  ...
  const chats = await db
    .chats()
    .find({ chatType: "Personal" }, { projection: { adminIds: 1 } })
    .toArray();
  const users = (await db.users().find({}).toArray()).filter((user) =>
    chats.some((chat) => !chat.adminIds.includes(user._id.toString()))
  );
