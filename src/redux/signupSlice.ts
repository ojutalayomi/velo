...
export const { setFormData, setLoading, setSuccess, setError, setError1, setErrors, setCurrentStep } = signupSlice.actions;

// Thunk action creators
export const handleValidation = (page: number): AppThunk<Promise<boolean>> => async (dispatch, getState) => {
...
export const handleNext = (step: number): AppThunk => async (dispatch, getState) => {
...
export const handlePrevious = (step: number): AppThunk => (dispatch, getState) => {
    const { currentStep } = getState().signups;
    if (currentStep) {
        dispatch(setCurrentStep(step));
    }
...
export const updateFormData = (data: Partial<FormData>): AppThunk => (dispatch) => {
