...
class ChatRepository {
  ...
  async getChatById(id: string): Promise<ChatAttributes | undefined> {
  ...
  async deleteMessageForMe(id: {[x: string]: string}): Promise<string> {
    // Delete a chat from the database
    const response = await fetch(`/api/chats/?chatId=${id}&messageId=${id}&userId=${id}&option=me`, {
        method: 'DELETE',
    });
    return await response.json();
  }
  ...
  async deleteMyMessage(id: {[x: string]: string}): Promise<string> {
    // Delete a chat from the database
    const response = await fetch(`/api/chats?chatId=${id}&messageId=${id}&userId=${id}&option=all`, {
        method: 'DELETE',
    });
    return await response.json();
  }
