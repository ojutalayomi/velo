...
import { BadgeInfo, BadgeX } from "lucide-react";
...
const VerifiedIcon = forwardRef<SVGSVGElement, ComponentPropsWithoutRef<"svg">>(
  ({ className, ...props }, ref) => (
    <svg
      ref={ref}
      viewBox="0 0 24 24"
      className={cn("w-4 h-4 text-brand fill-current", className)}
      {...props}
    >
...
// Map variants to icons (all are components)
const Icons = {
...
export const Statuser = forwardRef<SVGSVGElement, StatuserProps>(
  ({ variant = "verified", tooltipContent, ...props }, ref) => {
    ...
    const defaultTooltipContent = {
      ...
      footer: "Verification done by © 2025 VELO",
    };
