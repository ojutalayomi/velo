...
export const upload = multer({
  storage: multerS3({
    s3: s3Client,
    bucket: function (req: any, file: { fieldname: string; }, cb: (arg0: null, arg1: string) => void) {
    ...
    key: function (req: any, file: { originalname: string; }, cb: (arg0: null, arg1: string) => void) {
      const filename = timeFormatter1(new Date().toLocaleString()) + '/' + Math.round(Math.random() * 100000000000) + path.extname(file.originalname);
      cb(null, filename);
    }
  })
});
