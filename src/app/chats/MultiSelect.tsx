  ...
  const deleteMessages = () => {
    selectedMessages.map((msg) => {
      ...
      if (!currentMessage?.content.endsWith("≤≤≤")) {
        if (socket) {
          dispatch(
            updateConversation({ id: msg, updates: { lastMessage: "This msg was deleted" } })
          );
  ...
  return (
    <div className="bg-white/80 dark:bg-zinc-900/80 w-full max-w-full relative p-4 text-xs flex items-center justify-between">
      ...
      <div className="border rounded-md flex max-w-[70%] items-center justify-evenly">
        <div className="h-auto text-xs p-2 cursor-pointer" onClick={forwardMessages}>
          Forward
        </div>
        <div className="h-auto text-xs p-2 cursor-pointer" onClick={shareMessages}>
          Share
        </div>
        <div className="h-auto text-xs p-2 cursor-pointer" onClick={deleteMessages}>
          Delete
        </div>
      </div>
      <button className="cursor-pointer" onClick={() => dispatch(clearSelectedMessages())}>
        Cancel
      </button>
    </div>
  );
